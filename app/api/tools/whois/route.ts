...
function useRateLimit(request: NextRequest) { const ip = request.headers.get("x-forwarded-for")?.split(",")[0].trim() ?? request.headers.get("x-real-ip") ?? "unknown"; const now = Date.now(); const current = store.get(ip); const entry = !current || now - current.startedAt > windowMs ? { count: 0, startedAt: now } : current; if (entry.count >= limit) return { allowed: false, remaining: 0, resetIn: Math.ceil((windowMs - (now - entry.startedAt)) / 60_000) }; entry.count += 1; store.set(ip, entry); return { allowed: true, remaining: limit - entry.count, resetIn: Math.ceil((windowMs - (now - entry.startedAt)) / 60_000) }; }
...
export async function GET(request: NextRequest) {
  const rate = useRateLimit(request); if (!rate.allowed) return NextResponse.json({ error: `Rate limit exceeded. Try again in ${rate.resetIn} minutes.`, ...rate }, { status: 429 });
  try {
